...
## 상태 / 알려진 이슈 (2026-07-22)

> **보류 트랙.** 2026-07-22에 검증된 `dual_imu_cascade_pwm`에 집중하기로 결정
> 했다. 이 펌웨어는 첫 호버 이후 대안으로 재검토한다. 아래 미해결 이슈를 닫고
> 캐스케이드와의 중복을 헤더로 공유하기 전에는 능동 개발 대상이 아니다.

**아직 bench-only 후보다. 플래시·비행 검증을 하지 않았다.** 최초 이식은
`dual_imu_cascade_pwm`의 상당 부분을 복사해 왔는데, 그 시점 이후 캐스케이드에
들어간 개선들이 자동 전파되지 않았다. 현재까지 처리 상태:

- **[해결] RC 입력 하드닝 이식** — RC 워치독의 unsigned 언더플로(비행 중
  무작위 모터 컷을 유발하는 크래시 버그)를 캐스케이드와 동일한 부호 있는
  `rcTimedOut()`으로 교체했고, rc 시퀀스 파서에 `-`/`+`·ERANGE 거부와
  `rcSeqValid` 센티널을 이식했다. (컴파일 통과, 미비행)
- **[미해결] `gains` 명령 미구현** — `tune_pid.py`와 `docs/udp_protocol.md`는
  `gains` readback을 지원한다고 표기하지만 이 펌웨어는 응답하지 않는다.
  구현하거나 문서/스크립트에서 이 펌웨어 예외로 표기해야 한다.
- **[미해결] 텔레메트리 22필드 (캐스케이드는 30필드)** — 이 펌웨어는 `Armed`
  까지 22필드만 보낸다. 캐스케이드가 추가한 Tier 1 관측 필드(`Motor_M1..M4`,
  `PID_Loop_Hz`, `TgtRate_Roll/Pitch/Yaw`, 필드 23~30)가 없다. 지상 도구는
  짧은 패킷을 허용하므로(빠진 필드는 빈 값) 파싱은 깨지지 않지만, 모터 출력·
  루프 주파수·목표 각속도 관측이 불가능하다.
- **[미해결] body accel x/y 매핑이 검증된 캐스케이드와 반대 부호** — 아래
  "좌표계와 부호" 절의 벤치 검증(프로펠러 제거)을 반드시 수행해야 한다.
- **근본 원인/후속** — 공통 코드를 복붙 대신 헤더로 공유하면 향후 분기를 막을
  수 있다(writeMotor/desaturateMix/checkFreeze/융합/UDP 파서/워치독/rcTimedOut).
...
무시는 두 펌웨어가 동일하다. 텔레메트리는 이 펌웨어가 `Armed`까지 22필드,
캐스케이드는 Tier 1 필드까지 30필드로 **다르다**(위 "알려진 이슈" 참조).
